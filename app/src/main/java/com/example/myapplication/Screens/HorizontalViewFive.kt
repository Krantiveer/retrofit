...
class HorizontalViewFive : AppCompatActivity() {
    ...
    lateinit var receivedContentListFive: ArrayList<Content>
    private var sizeFive: Int? = null
    private var currentsizeFive: Int? = 0
    private var image_containFive: ImageView? = null
    private var progressBarFive: ProgressBar? = null
    protected var exoPlayerViewFive: PlayerView? = null
    protected var playerFive: ExoPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_horizontal_five_view)
    ...
    fun Fivescreen(){
        ...
        if (receivedContentListFive != null) {
            if (receivedContentListFive.get(0).contentType != "VIDEO") {
                ...
                Handler().postDelayed(Runnable {
                    currentsizeFive = currentsizeFive?.plus(1)
                    logicFive()
                ...
                image_containFive!!.setVisibility(View.GONE)
                exoPlayerViewFive!!.setVisibility(View.VISIBLE)

                initVideoPlayerFive(
                    receivedContentListFive.get(0).permaLink.toString(),
                    receivedContentListFive.get(0).format.toString()
                )
                // initVideoPlayer("http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4", "mp4")
    ...
    protected fun updateCardViewImage(url: String?, second: String) {
        ...
        else if (second.contentEquals("Five")) {
    ...
    */
    ...
    fun logicFive() {
        if (receivedContentListFive != null) {
            if (currentsizeFive!! >= size!!) {

                currentsizeFive = 0
                Log.i(TAG, "logic: " + currentsizeFive + sizeFive)
                Log.i(TAG, "restart: calling1")
            ...
            if (receivedContentListFive.get(currentsizeFive!!).contentType != "VIDEO") {
                exoPlayerViewFive!!.visibility = View.INVISIBLE

                image_containFive!!.setVisibility(View.VISIBLE)
                updateCardViewImage(receivedContentListFive.get(currentsizeFive!!).permaLink, "")
                var sec = receivedContentListFive.get(currentsizeFive!!).duration * 1000
                Log.i(TAG, "logic: " + currentsizeFive + sizeFive)
                Handler().postDelayed(Runnable {
                    currentsizeFive = currentsizeFive?.plus(1)
                    if (currentsizeFive!! >= sizeFive!!) {
                        ...
                        Log.i(TAG, "logic:1 " + currentsizeFive + sizeFive)
                ...
                exoPlayerViewFive!!.visibility = View.VISIBLE
                image_containFive!!.setVisibility(View.INVISIBLE)
                initVideoPlayerFive(
                    receivedContentListFive.get(currentsizeFive!!).permaLink.toString(),
                    receivedContentListFive.get(currentsizeFive!!).format.toString()
                )
                // initVideoPlayer("http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4", "mp4")
    ...
    fun initVideoPlayerFive(url: String?, type: String) {
        Log.i(TAG, "initVideoPlayer: $type")
        if (playerFive != null) {
            playerFive!!.stop()
            playerFive!!.release()
        ...
        playerFive = ExoPlayer.Builder(this).build()
        playerFive!!.setMediaItem(mediaItem)
        playerFive!!.trackSelector
        playerFive!!.prepare()
        playerFive!!.playWhenReady = startAutoPlay


        exoPlayerViewFive!!.player = playerFive

        playerFive!!.addListener(object : Player.Listener {
            ...
            override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
                ...
                if (playbackState == ExoPlayer.STATE_ENDED) {
                    ...
                    currentsizeFive = currentsizeFive?.plus(1)
                    Log.i(TAG, "onPlayerStateChanged: -->" + currentsizeFive)
                    if (sizeFive!! >= currentsizeFive!!) {
                        currentsizeFive = 0
                        if (receivedContentListFive.get(currentsizeFive!!).contentType == "VIDEO") {
                            image_containFive!!.setVisibility(View.GONE)
                            initVideoPlayerFive(
                            ...
                            exoPlayerViewFive!!.visibility = View.INVISIBLE
                            var sec =
                                receivedContentListFive.get(currentsizeFive!!).duration * 1000

                            updateCardViewImage(
                                receivedContentListFive.get(currentsizeFive!!).permaLink,
                                "Five"
                            )
    ...
    private fun releasePlayer() {
        ...
        if (playerFive != null) {
            playerFive!!.playWhenReady = false
            playerFive!!.stop()
            playerFive!!.release()
            playerFive = null
            exoPlayerViewFive!!.player = null
        }
    }
    ...
    fun restartFive() {
        ...
        if (receivedContentListFive != null) {
            if (receivedContentListFive.get(0).contentType != "VIDEO") {
                image_containFive!!.setVisibility(View.VISIBLE)
                exoPlayerViewFive!!.setVisibility(View.INVISIBLE)
                updateCardViewImage(receivedContentListFive.get(currentsize!!).permaLink,"Five")
                Handler().postDelayed(Runnable {
                    currentsizeFive = currentsizeFive?.plus(1)
                    logicFive()
